...
void TestSimalphaRes(double alpha){
...
//			E[i] = TMath::Power((ran1->Uniform()*specIntegralSignal*(signalIndex+1)+TMath::Power(Emin,signalIndex+1)),(1/(signalIndex+1)));
	...
	TH1D* energyint = new TH1D("energyint", "Energy", 30, 0, 1000);
	TH1D* energyrec = new TH1D("energyrec", "energyrec", 30, 0, 1000);
...
//	dist->cd(1);
	...
	energyrec->Fit("gaus");
	energyrec->Draw("");
	energyrec->SetTitle("Smearing of energy");
	energyrec->GetXaxis()->SetTitle("Energy (GeV)");
	energyint->Draw("same");
//	dist->cd(2);
//	timeint->Draw("");
