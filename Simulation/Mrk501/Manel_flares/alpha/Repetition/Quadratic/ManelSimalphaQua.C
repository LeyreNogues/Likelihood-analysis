...
void ManelSimalphaQua(double alpha){
...
//			std::cout << "Time = " << t[i] << " s"<< std::endl;
	...
	//Inject delay
	Double_t QGDelay= QGfixedPart*QGpar/(PlanckScale*1000.);
	std::cout << "QGDelay: " << QGDelay << std::endl;
//	std::cout << "QGDelay500: " << QGDelay*500*500 << std::endl;
	for(int i=0; i<numberOfEvents; i++){
		td[i]=t[i]+QGDelay*TMath::Power(E[i],2);
	}
...
//	timedel->Draw("same");
	...
	FILE *fout;
	fout = fopen(Form("Manel_flare_alpha%i_qua.txt", (int)alpha),"wb");
	for(int i = 0; i<numberOfEvents; i++)
	{
