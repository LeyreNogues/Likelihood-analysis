...
void SampleSimalpha(double alpha){
...
//	std::cout << "Integral Spectrum bkg = " << specIntegralbkg << std::endl;
   ...
   //LC inputs
	...
	TF1* PL_flare= new TF1("flare", powerLaw, Emin, Emax, 1);
	PL_flare->SetParameter(0, signalIndex);
	PL_flare->SetNpx(1000);
	TF1* PL_base= new TF1("base", powerLaw, Emin, Emax, 1);
	PL_base->SetParameter(0, bkgIndex);
	PL_base->SetNpx(1000);
	TF1* LC_flare= new TF1("base", "gaus(0)", tmin, tmax);
	LC_flare->SetParameters(1., gausMax, gausWidth);
	LC_flare->SetNpx(1000);
...
//			std::cout << "It's bkg" << std::endl;
			E[i] = PL_base->GetRandom(Emin, Emax);
			t[i] = ran12->Uniform(tmin, tmax);
			bkgtime->Fill(t[i]);
...
//			std::cout << "It's signal" << std::endl;
			E[i] = PL_flare->GetRandom(Emin, Emax);
			t[i] = LC_flare->GetRandom(tmin, tmax);
			signaltime->Fill(t[i]);
//			std::cout << "Energy = " << E[i] << " GeV"<< std::endl;
//			std::cout << "Time = " << t[i] << " s"<< std::endl;
		}
	...
	std::cout << "QGDelay: " << QGDelay << std::endl;
	for(int i=0; i<numberOfEvents; i++){
