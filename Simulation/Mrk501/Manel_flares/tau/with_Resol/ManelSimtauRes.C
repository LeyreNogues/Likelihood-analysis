...
void ManelSimtauRes(double tau){
   ...
   //LC inputs
	...
	Double_t PcTokm=3.085*TMath::Power(10,19);
	Double_t QGfixedPart=z*PcTokm/H0; //(ETA)
...
//	Double_t QGDelay= QGfixedPart*QGpar/PlanckScale;
	...
	for(int i = 0; i<numberOfEvents; i++)
		{
			Es[i]=E[i]*(1+d*resolution);
		}

	//Histograms of intrinsic (and delayed) time and energy
	...
	FILE *fout;
	fout = fopen(Form("Manel_flare_res_tau%i.txt", (int)tau),"wb");
	for(int i = 0; i<numberOfEvents; i++)
	{
