...
void simulation(Int_t num)
{
	...
	Double_t factorsig[numev] = 0;
	Double_t factor2sig[numev] = 0;
	Double_t factorbkg[numbkg] = 0;
	Double_t factor2bkg[numbkg] = 0;
	Double_t totalsig = 0;
	Double_t totalbkg = 0;
	for(int i = 0; i<numev; i++)
	{
		factorsig[i] = collar->Eval(Esig[i])/fmax;
		totalsig += factorsig[i];
	}
	for(int i = 0; i<numbkg; i++)
	{
		factorbkg[i] = collar->Eval(Ebkg[i])/fmax;
		totalbkg += factorbkg[i];
	}

	Double_t meanfactorsig = totalsig/numev;
	Double_t meanfactorbkg = totalbkg/numbkg;

	TH1D* weighthistosig = new TH1D("weighthistosig", "weighthistosig",  n, XX);
	TH1D* weighthistobkg = new TH1D("weighthistobkg", "weighthistobkg",  n, XX);
	for(Int_t i = 0; i<numev; i++)
	{
		factor2sig[i] = factorsig[i]/meanfactorsig;
		weighthistosig->Fill(Esig[i], factor2sig[i]/Esig[i]);
	}

	for(Int_t i = 0; i<numbkg; i++)
	{
		factor2bkg[i] = factorbkg[i]/meanfactorbkg;
		weighthistobkg->Fill(Ebkg[i], factor2bkg[i]/Ebkg[i]);
	}

	//weighthistosig->SetLineColor(4);
	//weighthistosig->Draw("sameEP");
	...
	for(Int_t a=0; a<numev; a++) //Only signal
	{
		...
		{
			plhisto->Fill(Esmearedsig[a], factor2sig[a]/Esmearedsig[a]);
		}
	}
	...
	Double_t Esim[f], tsim[f], factor2[f];
	Int_t nb, ns;
	nb = 0;
	ns = 0;
	...
	FILE *fout;
	fout = fopen(Form("/home/lnogues/workspace_cpp/LIVanalysis/Simulation/Mrk501/New/tau-80/flareA%i.txt", num),"wb");
	fprintf(fout,"%0.3lf \n", LCphotonssig+LCphotonsbkg);
	fprintf(fout,"%0.3lf \n", Fitphotonssig+Fitphotonsbkg);
	fprintf(fout,"%0.3lf \n", chipl[num]);
	for(int i = 0; i<f; i++)
	{
		fprintf(fout,"%0.3lf    %0.8lf    %0.8lf   \n", tsim[i], Esim[i], factor2[i]);
	}

	//Write a root file with the important plots
	TString rootoutfile(Form("/home/lnogues/workspace_cpp/LIVanalysis/Simulation/Mrk501/New/tau-80/SimulationA%i.root", num));
...
void AsepsimMrk()
{
	...
	//Mean computation
	Double_t sumtmean, sumtime, sumpl, sumratio, sumLC, sumPL, sumchipl, sumchilc;
	sumtmean = 0;
	sumtime = 0;
	sumpl = 0;
	sumratio = 0;
	...
	Double_t meantmean, meantime, meanpl, meanratio, meanLC, meanPL, meanchipl, meanchilc;
	...
	TH1D* indhisto = new TH1D("indhisto", "indhisto", 50, meanpl-0.3, meanpl+0.7);
	TH1D* chiplhisto = new TH1D("chiplhisto", "chiplhisto", 50, meanchipl-9, meanchipl+11);
	TH1D* chilchisto = new TH1D("chilchisto", "chilchisto", 50, meanchilc-4, meanchilc+6);
	...
	TH1D* ratiohisto = new TH1D("ratiohisto", "ratiohisto", 50, meanratio-0.3, meanratio+0.7);
	...
	ratiohisto->Draw("EP");
	big->SaveAs("./New/tau-80/Canvastau-80A.png");
