...
void simulation(Int_t num)
{
	...
	for(int i = 0; i<numev; i++)
	{
		smearhisto->Fill(Esmeared[i], factor2[i]/Esmeared[i]); // Yes weights at the moment
	}
	...
	FILE *fout;
	fout = fopen(Form("/home/lnogues/workspace_cpp/LIVanalysis/Simulation/Final/tau-80/flaretw%i.txt", num),"wb");
	fprintf(fout,"%0.3lf \n", LCphotons);
	fprintf(fout,"%0.3lf \n", Fitphotons);
	fprintf(fout,"%0.3lf \n", chipl[num]);
	for(int i = 0; i<i3; i++)
	{
		fprintf(fout,"%0.3lf    %0.8lf    %0.8lf   \n", tsimr[i], Esimr[i], factor2[i]);
	}

	//Write a root file with the important plots
	TString rootoutfile(Form("/home/lnogues/workspace_cpp/LIVanalysis/Simulation/Final/tau-80/Simulationtw%i.root", num));
	...
	energyhisto->Write();
	weighthisto->SetTitle("Weigted events");
	weighthisto->Write();
	laghisto->SetTitle(Form("Events with lag %f", tau));
	laghisto->Write();
	smearhisto->SetTitle("Smeared and weighted events");
	smearhisto->Write();
...
void shortsimtrue()
{
	...
	TCanvas* big = new TCanvas("tau-80simtw", "tau-80simtw", 1000, 1000);
	big->Divide(2,4);
	big->cd(1);
	...
	MeanEhisto->Draw("EP");
	big->SaveAs("./tau-80/Canvastau-80tw.png");
